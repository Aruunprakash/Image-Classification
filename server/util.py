...
def classify_image(image_base64_data, file_path=None):
    ...
    for img in imgs:
        ...
        prob_dictionary = {model_classes[i]: probabilities[i] for i in range(len(model_classes))}
        result.append({
            'class': str(__model.predict(final)[0]),
            'class_probability': prob_dictionary,  # <-- Cleaned up mapping
